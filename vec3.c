...
p_vec3 clone_vec3(p_vec3 vec) { return create_vec3(vec->x, vec->y, vec->z); }
...
p_vec3 create_random_vec3() { return create_random_interval_vec3(-100, 100); }
...
double length_squared_vec3(p_vec3 vec) { return dot_product_vec3(vec, vec); }

double length_vec3(p_vec3 vec) { return sqrt(length_squared_vec3(vec)); }
...
void unit_vector_vec3(p_vec3 vec) { divide_scalar_vec3(vec, length_vec3(vec)); }

p_vec3 random_in_unit_sphere() {
  p_vec3 vec = create_random_interval_vec3(-1, 1);
...
p_vec3 random_unit_vector_vec3() {
  p_vec3 vec = random_in_unit_sphere();
  unit_vector_vec3(vec);
  return vec;
}

p_vec3 random_in_hemisphere_vec3(p_vec3 normal) {
  p_vec3 in_unit_sphere = random_in_unit_sphere();
...
void reflect_vec3(p_vec3 vec, p_vec3 normal) {
  p_vec3 aux_normal = clone_vec3(normal);
...
p_vec3 refract_vec3(p_vec3 unit_vec, p_vec3 normal, double etai_over_etat) {
  p_vec3 inverse_unit_vec = clone_vec3(unit_vec);
  p_vec3 perpendicular_vec = clone_vec3(normal);
  p_vec3 parallel_vec = clone_vec3(normal);
